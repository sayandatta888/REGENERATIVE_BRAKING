...
[![Domain](https://img.shields.io/badge/Domain-Mechatronics%20%7C%20EV%20Engineering-1D9E75?style=flat-square)]()
[![Drive Cycle](https://img.shields.io/badge/Drive%20Cycle-UDDS%20%2F%20WLTP-185FA5?style=flat-square)]()
...
- [Subsystem Models](#subsystem-models)
...
=======================================================
  Drive cycle distance        : 11.94 km
  Total energy consumed       : 1842.3 Wh
  Energy recovered (regen)    : 412.7 Wh
  Regen recovery rate         : 22.4%
  Net energy used             : 1429.6 Wh
  Consumption                 : 119.7 Wh/km
  SoC change                  : 2.38%
  Average motor efficiency    : 92.1%
  Fault events                : 0
=======================================================
```
...
```
P_mech = T_demand × ω
P_elec = P_mech / (η_motor × η_inv)
P_batt = P_elec             [discharge]
```
...
```
P_mech_in  = T_regen × ω
P_recovered = P_mech_in × η_gen × η_inv × η_dcdc
P_batt     = −P_recovered   [charge]
```
...
```
1. Define EVParameters
        ↓
2. Generate UDDS drive cycle profile
        ↓
3. Initialize Battery, PID controller, state arrays
        ↓
4. For each timestep:
   ├── PID → force demand → torque demand
   ├── If braking → Regen controller → DC-DC → charge battery
   ├── If motoring → Motor → Inverter → discharge battery
   ├── Battery ECM update (SoC, V_terminal, fault check)
   └── Vehicle dynamics integration (Euler forward)
        ↓
5. Post-process: plot 7 result charts
        ↓
